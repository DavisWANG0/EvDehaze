...
class GaussianDiffusion:
    ...
    def encode_first_stage(self, y, first_stage_model, up_sample=False):
        ...
        if first_stage_model is None:
            ...
            # 默认 no_grad（与原行为一致）；当 first_stage_model 标记 grad_encode=True
            # （例如 FrozenVAEResampler）时放开，使 encode 处于可求导上下文。
            grad_encode = getattr(first_stage_model, 'grad_encode', False)
            enc_ctx = contextlib.nullcontext() if grad_encode else th.no_grad()
            with enc_ctx:
    ...
    def training_losses(
        ...
        if self.loss_type == LossType.MSE or self.loss_type == LossType.WEIGHTED_MSE:
            ...
            # 当 grad_encode=True（可学习 Up 经由冻结 VAE 反传）时，把回归目标 detach，
            # 防止 MSE 通过"移动目标"塌缩；Up 仍能从 z_y / z_t 通路获得梯度。
            grad_encode = getattr(first_stage_model, 'grad_encode', False)
            z_start_tgt = z_start.detach() if grad_encode else z_start
            z_y_tgt = z_y.detach() if grad_encode else z_y
            target = {
                ModelMeanType.START_X: z_start_tgt,
                ModelMeanType.RESIDUAL: z_y_tgt - z_start_tgt,
