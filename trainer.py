...
def normalized_rgb_y(pred):
    coeff = pred.new_tensor([65.481, 128.553, 24.966]).view(1, 3, 1, 1)
    return (pred * 0.5 + 0.5).mul(coeff).sum(dim=1, keepdim=True) / 255.0 + (16.0 / 255.0)


def normalized_rgb_y_mse(pred, target):
    y_delta = normalized_rgb_y(pred) - normalized_rgb_y(target)
    return y_delta.square().flatten(1).mean(dim=1)


def normalized_lowfreq_y_mse(pred, target, sizes):
    pred_y = normalized_rgb_y(pred.float())
    target_y = normalized_rgb_y(target.float())
    losses = []
    for size in sizes:
        size = int(size)
        pred_low = F.adaptive_avg_pool2d(pred_y, (size, size))
        target_low = F.adaptive_avg_pool2d(target_y, (size, size))
        losses.append((pred_low - target_low).square().flatten(1).mean(dim=1))
...
class TrainerBase:
    ...
    def build_model(self):
        ...
        if self.configs.model.ckpt_path is not None:
            ...
            # tolerant_load: drop checkpoint keys whose shape no longer matches the model
            # (e.g. a redesigned event encoder); those modules are re-initialized.
            if self.configs.model.get('tolerant_load', False):
                ckpt = {k.replace('module.', '') if k.startswith('module.') else k: v
                        for k, v in ckpt.items()}
                model_sd = model.state_dict()
                dropped = [k for k, v in ckpt.items()
                           if k in model_sd and v.shape != model_sd[k].shape]
                for k in dropped:
                    ckpt.pop(k)
                if self.rank == 0 and dropped:
                    self.logger.info(f"[tolerant_load] dropped {len(dropped)} shape-mismatched keys "
                                     f"(re-initialized): e.g. {dropped[:3]}")
    ...
    def logging_image(self, im_tensor, tag, phase, add_global_step=False, nrow=8):
        ...
        # 可视化健壮性：扩散中间量(高 kappa 噪声经 VAE 解码，AMP 下可能出现 Inf/NaN)
        # 若不净化，make_grid(normalize=True) 会把 Inf 传播到输出，导致 img_as_ubyte 抛
        # "Images of type float must be between -1 and 1." 进而使 rank0 退出、NCCL watchdog 卡死拖崩 DDP。
        im_tensor = torch.nan_to_num(im_tensor.detach().float(), nan=0.0, posinf=0.0, neginf=0.0)
        im_tensor = vutils.make_grid(im_tensor, nrow=nrow, normalize=True, scale_each=True) # c x H x W
        im_tensor = im_tensor.clamp(0.0, 1.0)
...
class TrainerDifIR(TrainerBase):
    ...
    def build_model(self):
        ...
        if self.configs.autoencoder is not None:
            # ⭐ 对于SD VAE，不需要torch.load（diffusers会自动加载）
            is_sd_vae = 'sd_vae' in self.configs.autoencoder.target.lower()
            if not is_sd_vae:
            ...
            # ⭐ SD VAE已经在__init__中加载权重，不需要再load_model
            if not is_sd_vae:
    ...
    def training_step(self, data):
        ...
        for jj in range(0, current_batchsize, micro_batchsize):
            ...
            # ⭐ 兼容不同的VAE：VQ-VAE有ddconfig，SD VAE直接用downsample_factor
            if hasattr(self.autoencoder, 'downsample_factor'):
                # SD VAE
                latent_downsamping_sf = self.autoencoder.downsample_factor
                noise_chn = self.autoencoder.embed_dim
            else:
                # VQ-VAE
                latent_downsamping_sf = 2**(len(self.configs.autoencoder.params.ddconfig.ch_mult) - 1)
                noise_chn = self.configs.autoencoder.params.embed_dim if 'autoencoder' in self.configs else micro_data['gt'].shape[1]
...
class TrainerDifIRLPIPS(TrainerDifIR):
    def backward_step(self, dif_loss_wrapper, micro_data, num_grad_accumulate, tt):
        ...
        with context():
            ...
            if len(loss_coef) > 2 and loss_coef[0] > 0 and loss_coef[2] > 0:
                pixel_l1 = (x0_pred - micro_data['gt']).abs().flatten(1).mean(dim=1)
                losses["l1"] = pixel_l1.to(losses["mse"].dtype) * loss_coef[2]

            y_mse_coef = self.configs.train.get('y_mse_coef', 0.0)
            if y_mse_coef > 0:
                losses["ymse"] = normalized_rgb_y_mse(
                    x0_pred,
                    micro_data['gt'],
                ).to(losses["mse"].dtype) * y_mse_coef

            lowfreq_y_mse_coef = self.configs.train.get('lowfreq_y_mse_coef', 0.0)
            if lowfreq_y_mse_coef > 0:
                lowfreq_y_sizes = self.configs.train.get('lowfreq_y_sizes', [4, 8, 16, 32])
                losses["lfymse"] = normalized_lowfreq_y_mse(
                    x0_pred,
                    micro_data['gt'],
                    lowfreq_y_sizes,
                ).to(losses["mse"].dtype) * lowfreq_y_mse_coef
            ...
            if "l1" in losses:
                losses["loss"] = losses["loss"] + losses["l1"]
            if "ymse" in losses:
                losses["loss"] = losses["loss"] + losses["ymse"]
            if "lfymse" in losses:
                losses["loss"] = losses["loss"] + losses["lfymse"]
            loss = losses['loss'].mean() / num_grad_accumulate
        if self.amp_scaler is None:
            loss.backward()
        else:
...
class TrainerEventDifIRLPIPS(TrainerDifIRLPIPS):
    """
    Event-aware DifIR + LPIPS trainer.

    与 TrainerDifIRLPIPS 相同，区别在于训练时把 ``data['event']`` 一并放入
    ``model_kwargs``，使 UNet 的 ``event_encoders`` / ``EventCrossAttention``
    接收到事件信号。

    依赖：
      * ``data['event']`` 形状 ``(B, C_event, H, W)``，与 ``lq/gt`` 同分辨率，
        配合 ``type: paired_aligned`` 数据集使用。
      * UNet 的 ``forward(..., event=...)``（见 ``models/evdehaze.py``）。
    """
    ...
    def training_step(self, data):
        ...
        for jj in range(0, current_batchsize, micro_batchsize):
            ...
            # 兼容不同 VAE：VQ-VAE 走 ddconfig，SD VAE 走 downsample_factor
            if hasattr(self.autoencoder, 'downsample_factor'):
                latent_downsamping_sf = self.autoencoder.downsample_factor
                noise_chn = self.autoencoder.embed_dim
            else:
                latent_downsamping_sf = 2 ** (len(self.configs.autoencoder.params.ddconfig.ch_mult) - 1)
                noise_chn = (
                    self.configs.autoencoder.params.embed_dim
                    if 'autoencoder' in self.configs
                    else micro_data['gt'].shape[1]
                )
            ...
            # ⭐ 与父类唯一不同：把 event 一起喂给 UNet
            if self.configs.model.params.cond_lq:
                model_kwargs = {'lq': micro_data['lq']}
                if 'mask' in micro_data:
                    model_kwargs['mask'] = micro_data['mask']
                if 'event' in micro_data:
                    model_kwargs['event'] = micro_data['event']
            else:
                model_kwargs = None
            ...
            if last_batch or self.num_gpus <= 1:
                losses, z0_pred, z_t = self.backward_step(
                    compute_losses, micro_data, num_grad_accumulate, tt,
                )
            else:
                with self.model.no_sync():
                    losses, z0_pred, z_t = self.backward_step(
    ...
    def validation(self, phase='val'):
        """
        与 TrainerDifIR.validation 完全相同，唯一差别：把 ``data['event']`` 也塞进
        ``model_kwargs``。否则验证时模型同样收到零事件，PSNR 会被低估。
        """
        if self.rank == 0:
            if self.configs.train.use_ema_val:
                self.reload_ema_model()
            ...
            if 'gt' in data:
                ...
                mean_lpips /= len(self.datasets[phase])
                self.logger.info(f'Validation Metric: PSNR={mean_psnr:5.2f}, SSIM={mean_ssim:6.4f}, LPIPS={mean_lpips:6.4f}...')
                self.logging_metric(mean_psnr, tag='PSNR', phase=phase, add_global_step=False)
...
class TrainerEventResampleLPIPS(TrainerEventDifIRLPIPS):
    """
    Event-aware DifIR + LPIPS，在冻结 VAE 前后挂 bicubic 像素重采样以适配 diffusion-based
    和 transformer-based 模型的公平比较。For numerical results, to ensure a fair comparison
    between diffusion-based and transformer-based methods, we uniformly follow prior
    practice by downsampling the data to 128×128 for training and metric computation.
    For the final visualizations, use visualize.py at the original image resolution.

        input 128 ─(bicubic Up)─▶ 256 ─[Frozen VAE.encode]─▶ latent 64
                                                              │  diffusion UNet (image_size=64)
        output 128 ◀─(bicubic Down)─ 256 ◀─[Frozen VAE.decode]─ latent 64

    - VAE 全程冻结；可训 = UNet + latent residual correction（x0 latent 偏置修正）。
    - latent=64 时 EvDehaze 内 event 融合的固定 level 尺寸 {64,32,16,8} 自然对齐。
    - 多卡：latent correction 不在 DDP 内，迭代末手动 all-reduce 其梯度。
    """

    def build_model(self):
        super().build_model()
        from models.resample import FrozenVAEResampler
        assert self.autoencoder is not None, "FrozenVAEResampler 需要非空 VAE"
        rs_cfg = self.configs.get('resample', None) if hasattr(self.configs, 'resample') else None
        latent_correction = bool(rs_cfg.get('latent_correction', False)) if rs_cfg else False
        latent_kwargs = dict(rs_cfg.get('latent_kwargs')) if (rs_cfg and rs_cfg.get('latent_kwargs', None)) else None
        scale = int(rs_cfg.get('scale', 2)) if rs_cfg else 2
        embed_dim = self.configs.autoencoder.params.embed_dim
        wrapper = FrozenVAEResampler(self.autoencoder, embed_dim=embed_dim,
                                     latent_correction=latent_correction, latent_kwargs=latent_kwargs,
                                     scale=scale)
        wrapper.cuda()
        if rs_cfg and rs_cfg.get('ckpt_path', None):
            rs_ckpt = Path(str(rs_cfg.ckpt_path))
            sd = torch.load(rs_ckpt, map_location=f"cuda:{self.rank}")
            if isinstance(sd, dict) and 'state_dict' in sd:
                sd = sd['state_dict']
            wrapper.load_resample_state_dict(sd)
            if self.rank == 0:
                self.logger.info(f"[FrozenVAEResampler] loaded latent correction from {rs_ckpt}")
        self.autoencoder = wrapper
        if self.rank == 0:
            n_rs = sum(p.numel() for p in wrapper.trainable_parameters() if p.requires_grad) / 1e6
            self.logger.info(
                f"[FrozenVAEResampler] bicubic resample (frozen), trainable latent correction = {n_rs:.3f}M, "
                f"downsample_factor={wrapper.downsample_factor}")

    def setup_optimizaton(self):
        # UNet(DDP) + latent correction 一起优化；bicubic 重采样无参数、VAE 冻结，均不入组
        latent_lr_mult = self.configs.train.get('latent_lr_mult', 1.0)
        latent_params = [p for p in self.autoencoder.latent_parameters() if p.requires_grad]
        param_groups = [
            {
                'params': list(self.model.parameters()),
                'lr': self.configs.train.lr,
                'initial_lr': self.configs.train.lr,
            }
        ]
        if latent_params:
            param_groups.append({
                'params': latent_params,
                'lr': self.configs.train.lr * latent_lr_mult,
                'initial_lr': self.configs.train.lr * latent_lr_mult,
            })
        ...
        if self.rank == 0:
            self.logger.info(
                f"[FrozenVAEResampler] optimizer lr: unet={self.configs.train.lr:.2e}, "
                f"latent={self.configs.train.lr * latent_lr_mult:.2e}")
        self.amp_scaler = amp.GradScaler() if self.configs.train.use_amp else None
        # 与 TrainerDifIR 一致的 cosine 调度（train 循环依赖 self.lr_scheduler）
        if self.configs.train.lr_schedule == 'cosin':
            self.lr_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
                optimizer=self.optimizer,
                T_max=self.configs.train.iterations - self.configs.train.warmup_iterations,
    ...
    def _allreduce_resample_grads(self):
        for p in self.autoencoder.trainable_parameters():
            if p.requires_grad and p.grad is not None:
                dist.all_reduce(p.grad, op=dist.ReduceOp.SUM)
                p.grad /= self.num_gpus
    ...
    def training_step(self, data):
        ...
        for jj in range(0, current_batchsize, micro_batchsize):
            ...
            # FrozenVAEResampler 暴露 downsample_factor=2 -> latent 分辨率 = 128 // 2 = 64
            latent_downsamping_sf = self.autoencoder.downsample_factor
            noise_chn = self.autoencoder.embed_dim
            latent_resolution = micro_data['gt'].shape[-1] // latent_downsamping_sf
            noise = torch.randn(
        ...
        # latent correction 不在 DDP 内：手动跨卡同步其梯度
        if self.num_gpus > 1:
            self._allreduce_resample_grads()

        # ---- NaN 防护 + 梯度裁剪: 避免 amp/fp16 偶发溢出污染权重导致永久发散 ----
        # 先 unscale 才能正确检查/裁剪真实梯度量级
        if self.configs.train.use_amp:
            self.amp_scaler.unscale_(self.optimizer)

        clip_params = [p for p in self.model.parameters() if p.grad is not None]
        clip_params += [p for p in self.autoencoder.trainable_parameters() if p.grad is not None]

        finite = all(torch.isfinite(p.grad).all() for p in clip_params)
        # 多卡: 任一 rank 出现非有限梯度 -> 全体一起 skip, 否则 update_ema 的 barrier 会死锁
        if self.num_gpus > 1:
            flag = torch.tensor([1.0 if finite else 0.0], device=f"cuda:{self.rank}")
            dist.all_reduce(flag, op=dist.ReduceOp.MIN)
            finite = flag.item() > 0.5

        if finite:
            torch.nn.utils.clip_grad_norm_(clip_params, max_norm=1.0)
        elif self.rank == 0:
            self.logger.info(f"[nan-guard] skip optimizer step @ iter {self.current_iters}: non-finite grad")

        if self.configs.train.use_amp:
            # 已手动 unscale_; finite=False 时 step 内部会因 inf 标记自动跳过参数更新
            self.amp_scaler.step(self.optimizer)
            self.amp_scaler.update()
        elif finite:
            self.optimizer.step()

        # 一次清空 UNet + latent correction 的梯度
        self.optimizer.zero_grad()

        # 仅在本步有效更新后才更新 EMA(否则会把坏权重错误平滑进 EMA)
        if finite and hasattr(self.configs.train, 'ema_rate'):
            self.update_ema_model()

    def save_ckpt(self):
        super().save_ckpt()
        if self.rank == 0:
            rs_path = self.ckpt_dir / 'resample_{:d}.pth'.format(self.current_iters)
            torch.save(self.autoencoder.resample_state_dict(), rs_path)

    def resume_from_ckpt(self):
        super().resume_from_ckpt()
        if self.configs.resume:
            cand = Path(self.configs.resume).parent / Path(self.configs.resume).name.replace('model_', 'resample_')
            if cand.is_file():
                sd = torch.load(cand, map_location=f"cuda:{self.rank}")
                self.autoencoder.load_resample_state_dict(sd)
                if self.rank == 0:
                    self.logger.info(f"=> Loaded resample (up/down) from {cand}")
